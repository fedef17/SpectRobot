...
def mask_and_check_pixels(pixels):
    i=0
    for pix in pixels:
        ...
        cond2 = (pix.observation.spectrum < -3.e-8)
        pix.observation.mask[cond2] = 0
        cond2 = (pix.observation.spectrum > 1.e-6) & (gri < 3200.)
        pix.observation.mask[cond2] = 0
        cond2 = (pix.observation.spectrum > 3.e-6)
        pix.observation.mask[cond2] = 0
    ...
    pixels = [pix for pix in pixels if pix.limb_tg_alt > 400.]
...
for yea in [2006,2007]:
    ...
    for lat1,lat2 in zip(lats[:-1], lats[1:]):
        ...
        if all_seqs.has_key(latsss):
            ...
            continue
            #for seq in year_sequences[yea][latsss]:
            #    print(seq['n_pixels'], seq['szas']

        sequa = all_seqs[latsss]
        #sequa.sort(key = lambda x: x['n_pixels'])
        if len(sequa) > 10:
            #sequa = [se for se in sequa if se['n_pixels']]
            sequa.sort(key = lambda x: x['szas'][1])
            sequa = sequa[:10]
            #sequa.sort(key = lambda x: x['n_pixels'])
